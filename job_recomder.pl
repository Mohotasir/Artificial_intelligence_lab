...
add_user(Name, Skills, Experience, PreferredLocation, InterestFields, BlacklistedCompanies) :-
    \+ user(Name, _, _, _, _, _),  % Check user doesn't already exist
    assertz(user(Name, Skills, Experience, PreferredLocation, InterestFields, BlacklistedCompanies)),
    write('User added successfully.'), nl.

add_user(Name, _, _, _, _, _) :-
    user(Name, _, _, _, _, _),
